...
This chapter explores the state of the art on application runtime manipulation. From existing solutions and languages, we identify three main problems based on our problem statement that arise while modifying an application runtime~(Section \ref{sec:bootstrapping_problems}):

\begin{description}
\item[Flexibility to change.] How malleable is the language to introduce a change on it. Flexibility involves if the language definition is properly structured and abstracted, if it is hardcoded or not, if it is safe to apply a change without breaking the whole runtime.
\item[Mixture of low-level and high-level concerns.] The coupling between the language and the \VM may prevent changes in the application runtime if they are not well factored and explicit to the language developer.

...
\chapter{Modifying the Language Runtime}
\chaplabel{state_modifying}

Evolving a language can take the path of modifying its language runtime, particularly the concepts the language exposes to the developer and its semantics.
We identify three main problems in taking such path of evolution~(Section \ref{sec:bootstrapping_problems})

\begin{description}
\item[Flexibility to change.] How malleable is the language to introduce a change on it. If the language definition is properly structured and abstracted, or not hardcoded.
\item[Mixture of low-level and high-level concerns.] The coupling between the language and the \VM may prevent some changes if they are not well factored and explicit to the language developer.
\item[Tool support.] What is the existing tool support to modify the language itself. Here, the worse case scenario is the usage of the low-level \VM tools for that purpose.
\end{description}

To introduce changes in a language's semantics, several solutions are proposed, mainly based on metaprogramming and \emph{reflection}~(Section \ref{sec:related_work_changing}). These metaprogramming models allow either the modification of the language interpreter semantics or the scoping of modifications with one main objective: performing such modifications safely \ie not shooting in their own foot by modifying the runtime they are running on.

Another direction is taken recently by metacircular runtimes~(Section \ref{sec:metacircular_runtimes}). Metacircular runtimes aim to ease the modification of a runtime by means of simplifying the language-\VM relation for development. This simplification comes usually by expressing a \VM in the same language it interprets at the end.


Finally, this chapter concludes by \gp{I lack a way to conclude this chapter. I could put some problem statement...}



\section{Problems of Language Runtimes Modification} \label{sec:bootstrapping_problems}

We identify three main limitations when we aim to change a language runtime. First, how flexible a language definition is to be changed and adapted without running into inconsistent states. Second, the unclear mixture of \VM and language concerns prevents us to extract the language definition and easily replace it. Finally, the differences in the abstraction levels and tool support between the language we are building and the language we use to build it makes it a challenging task.


\subsection{Flexibility to Safely Change a Language Runtime}

The \VM is often the component in charge of initialising the language runtime. This decision is indeed practical as the \VM can safely initialise the language structures and solve the language bootstrapping issues avoiding recursions~\cite{Kicz91a}~(\eg create the first class without a class). This language runtime must comply at runtime to the execution model imposed by the \VM. This coupling is indeed necessary to run a program but does often remain hidden in hardcoded assumptions.
...
To illustrate this second problem, let us consider the code in Figure~\ref{code:logic_dup} that creates a \ct{Dictionary} object~(a hash map object). A language runtime that is defined by a \ct{Dictionary} object to keep \eg a map of global objects, must execute the code in the figure to create the corresponding instance. However, since the language runtime and the \VM are in middle of their initialization, the \VM cannot execute this code as it is, and thus it cannot enforce its own invariants. The state of the art \VMs will provide an alternative low-level representation of the same code respecting the same invariants. This introduces a redundancy: the \VM and the language have different code to honor the same invariants.
...
\end{code}
\end{subfigure}
\caption{\textbf{Code to create a \ct{Dictionary} object.} To the left, the code is written in Smalltalk. To the right, a function with the similar semantics, but written in C, duplicates this behavior.\label{code:logic_dup}}
\end{figure}
...
\section{Changing Language Semantics}\label{sec:related_work_changing}
...
\section{Metacircular Runtimes}\label{sec:metacircular_runtimes}
...
\gp{conclude on what the related work does and does not for language runtime modification}

\chapter{Application Tailoring}
\chaplabel{state_tailoring}

In the context of this thesis, we explore application tailoring as one possible evolution for a language runtime in the context of devices with limited infrastructures~(either hardware or software).
...
\chaplabel{state_virtualization}
...
Operating System virtualization technology is characterized by the existence of an \emph{hypervisor}~(named after the Operating System \emph{supervisor} that controls the OS processes\gp{check well that name}). The hypervisor is the \VM component that allows one to observe or control the internals of one or many \VMs. A \VM hypervisor gives us, amongst others, the following services:
...
In this chapter, and with the objective of doing runtime manipulation, we study in the following sections techniques that to our understanding are virtualization-related techniques applied to programming languages runtimes: application co-existence~(Section \ref{sec:virtualization_coexistence}) and application manipulation~(Section \ref{sec:virtualization_control}). Finally, we conclude by presenting the aspects of these solutions that inspired our solution following in the next chapters.%We divide theses works in two main topics that are of interest for us in the subject of runtime manipulation. First, we present related work that targets co-location of language runtimes on the same \VM. We continue this section by presenting related work on the subject of runtime control.

\section{Application Co-existence}\label{sec:virtualization_coexistence}
...
\section{Application Manipulation}\label{sec:virtualization_control}
...
\gp{what are the interesting aspects that inspired our solution}

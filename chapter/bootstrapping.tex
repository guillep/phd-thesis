...
\item[The language elements should be self-described.] The input language definition must include a description of the classes, methods, traits and/or objects that are part of the language runtime to build. This description should be expressed in the same language we are building. From now on we will call these the \emph{base-level} entities.
...
\item[The output is a graph of objects.] The runtime representation that outputs this process must be made of a graph of objects \ie the classes, methods, threads and other objects that allow programs to run. These elements are created by the bootstrap inside an application runtime. They form a graph because these objects are interconnected as a result of the bootstrap process.
...
In this section we illustrate bootstrapping through an example: the bootstrap process for the Pharo language. Our bootstrap process requires a description of the elements of the language and the procedure to create them. We will call such a description the \emph{language definition} from now on. A language definition is written in the same language it defines at the end. For example, Figure \ref{fig:example_language_definition} shows an excerpt of Pharo's language definition written in Pharo. The bootstrap loads the language definition, available as source code files, and creates the bootstrapped application runtime. Notice that while such a bootstrap is not difficult to express per se, it raises the question of \emph{how it can be executed}. This is challenging especially, since for example we need classes to be defined to execute this exact bootstrap description. This question leads to ask ourselves about the infrastructure required to be able to manage this and other different bootstraps. The sections that follow describe how \Vtt provides an infrastructure that tackles these problems.
...
Bootstrapping requires a language runtime to be self-described. That is, there should exist a definition of the language expressed in the language it defines.
The bootstrap process takes as input this language definition and reifies them as abstract syntax trees~(ASTs) of the language elements~(\eg classes and methods).
ASTs ease the bootstrap process and provide it with information for compilation and class creation \eg variable and class names, superclasses, how to bind names during compilation. Figure \ref{fig:hazelSpecModel} illustrates our basic AST model containing information about how to initialize the language runtime, its initialization order and which are the classes/traits and methods it has to build.
...
For better understanding, we can decompose the elements of a language definition in two main components:
...
A language that contains both introspection and full intercession facilities is a fully reflective language. A fully reflective language does not only have the minimal set of elements to run, but also the minimal to be autonomous: it can create/load classes and methods without any external component~(compiler, class builder, interpreter). Thus, a language definition has direct impact on the design of the language we bootstrap. Figure \ref{fig:phases} shows the stages of a bootstrap process as it installs classes or packages inside the language runtime. When the bootstrap process starts, the language runtime is not yet able to execute code by itself \eg it cannot resolve the method lookup because the class hierarchy is not created. As we install packages or classes, we arrive at the point where the language runtime contains already the minimal set of elements it needs for execution, the \emph{execution point}. Later on, we can install reflective features into the language runtime to get it inside the \emph{reflective spectrum}.

...
This paper describes the \emph{run-fail-grow} (RFG) technique: an alternative solution to dead code elimination that identifies at runtime those code units that are actually used in an application.
...
In summary, this paper \emph{contribution} is two-fold. We contribute our run-fail-grow technique which is applicable to dynamic languages with no type annotations, has natural support for reflection, and is customizable by specifying a seed. Our second contribution is the description of its implementation, detailing those elements that should be available in a programming environment for it, for those interested in implementing such a solution.
...
Additionally, in both situations the deployed application tends to occupy more secondary memory than necessary since the code units should be available to be loaded.
...
\begin{figure*}[ht]
\begin{center}
\includegraphics[width=.244\linewidth]{example_before}
\includegraphics[width=.25\linewidth]{example_starting_point}
\includegraphics[width=.25\linewidth]{example_dnu_trap_start}
\includegraphics[width=.25\linewidth]{example_shadow_trap}
\includegraphics[width=.246\linewidth]{example_dnu_trap}
\includegraphics[width=.25\linewidth]{example_finished}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=0.6\linewidth]{tornado_components2}
...
\begin{description}
\item[Process reification provides execution control.] Pharo provides already support for execution control in its runtime. It reifies processes and method activations~(\ie instances of \ct{Process} and \ct{MethodContext} respectively) allowing one to manipulate them as simple objects. However, by itself Pharo does not provide the ability to control its execution remotely as it is needed by Tornado. We developed Oz object spaces to cover this missing feature.

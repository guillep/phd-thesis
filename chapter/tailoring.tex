...
Particularly, we can find the case where no method is found in the reference application due to either a bug in the application or a design choice to use the dynamic features of Pharo. In those cases Tornado sends the \ct{doesNotUnderstand:} message to the receiver object~(an equivalent to \eg Ruby's \ct{method\_missing} and Python's \ct{\_\_getattr\_\_}) which in turn could be handled by the application or throw a corresponding exception.

\item[Method Invoked Trap.] We implemented method invoked traps in Tornado using method proxies. Method proxies are placed in the method dictionaries of classes containing overridden methods, taking the place of the original method.  When Tornado installs a class into the nurtured runtime that contains overridden methods in the reference object space, it installs into this class a method proxy for each of its overridden methods. This trap is triggered whenever the method proxy it is about to be executed. The handler of this trap compiles a new method with the same source as the corresponding method from the reference application and installs it inside the nurtured runtime.
...
\begin{table}[ht]
 	...
 	\begin{tabular}{lccccc}
		...
		\bottomrule
 	\end{tabular}
 	\caption{\small\textbf{Results of the tailored experiments.} Sizes are displayed in KB. The percentage of saved space does not take into account the seed, as it is not subject of Tornado and it is shared by both the reference and nurtured runtime.}
 	\label{tb:results}
 \end{table}

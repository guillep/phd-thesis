...
The dynamic nature of our solution allows its usage in dynamically-typed languages, and applications using reflection. Our solution does not need to modify the original application because our run-fail-grow works outside of the nurtured application, transparently.
...
Once finished, the nurtured application is ready to be deployed onx target devices. Figure~\ref{fig:runfail} depicts the basics of our run-fail-grow approach.
...
We implemented RFG in a tool called Tornado based in \Vtt. With \Vtt, we are able to monitor the nurtured application's execution using the execution cycles. Ghost-like proxies represent traps that detect missing code units during execution. We use Mirrors to install required code and restart the execution as it is needed.
